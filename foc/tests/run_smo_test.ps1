...
Write-Output "SKIP: legacy SMO assertions await the SMO rewrite"
return
$repoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path
$sourceFiles = @(
    (Join-Path $repoRoot "foc/tests/foc_smo_test.c"),
...
foreach ($backend in @("FLOAT", "FIXED")) {
    $testExe = Join-Path $env:TEMP "foc_smo_$backend.exe"
    ...
    Write-Output "$backend SMO tests passed"
}
