...
$sourceFiles = @(
    (Join-Path $repoRoot "foc/tests/foc_smo_switch_feedback_test.c"),
...
foreach ($backend in @("FLOAT", "FIXED")) {
    $testExe = Join-Path $env:TEMP "foc_smo_switch_$backend.exe"
    $compileArgs = @(
        "-std=gnu11", "-O0", "-Wall", "-Wextra", "-Werror",
        "-DFOC_TRIG_BACKEND=1", "-DFOC_NUMERIC_$backend=1",
    ...
    Write-Output "$backend SMO switch feedback test passed"
}
