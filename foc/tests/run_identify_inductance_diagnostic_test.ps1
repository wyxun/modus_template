...
$sourceFiles = @(
    (Join-Path $repoRoot "foc/tests/identify_inductance_diagnostic_test.c"),
...
foreach ($backend in @("FLOAT", "FIXED")) {
    $testExe = Join-Path $env:TEMP "identify_inductance_diag_$backend.exe"
    ...
    Write-Output "$backend inductance diagnostic tests passed"
}
